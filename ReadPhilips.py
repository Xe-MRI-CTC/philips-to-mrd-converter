...
class PhilipsData():
    ...
    def __init__(self, fname):        
        ...
        self.ApplydoDownsampling = False
    ...
    def compute(self):
        ...
        if self.readType == 0:
            ...
            if os.path.exists(filename_extcase(self.base+'.data')) and os.path.exists(filename_extcase(self.base+'.list')) \
                ...
                try:
                    dat_noi = dat_noi.astype(np.float32).squeeze()
                except:
                    pass
                ...
                # YCC start
                try:
                    dat_phc = dat_phc.astype(np.float32).squeeze()
                except:
                    pass
                # YCC achieve
...
##############################################################################            
            ...
            if os.path.exists(filename_extcase(self.base+'.lab')) and os.path.exists(filename_extcase(self.base+'.raw')) \
                ...
                try:
                    ...
                    pass
                try:
                    dat_phc = dat_phc.astype(np.complex64).squeeze()
                except:
                    pass
                try:
                    dat_noi = dat_noi.astype(np.complex64).squeeze()
                except:
                    pass
                header.update(hdr)
