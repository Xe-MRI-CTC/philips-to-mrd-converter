...
class Config():
    ...
    def update(self, dl, rls, mrdHeader): # default values hard coded for XeCTC acquisition at CCHMC
        if 'Dissolved_Xe'.lower() or 'CPIR'.lower() in rls.header['sin']['scan_name'][0][0].lower(): # two CPIR versions use golden means
            self.traj_type = 1
        ...
        if 'Dissolved_Xe'.lower() or 'CPIR'.lower() in rls.header['sin']['scan_name'][0][0].lower(): # two CPIR versions are cartesian (no trajs) for added spec
            self.ext_traj = True

        if 'Dissolved_Xe'.lower() or 'CPIR'.lower() in rls.header['sin']['scan_name'][0][0].lower(): # two CPIR versions collect diss/gas/off res
            self.contrast_order = [2, 1, 3]

        if 'Dissolved_Xe'.lower() or 'CPIR'.lower() in rls.header['sin']['scan_name'][0][0].lower(): # two CPIR versions containing bonus spectra
