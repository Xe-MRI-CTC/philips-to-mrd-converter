...
class Config():
    ...
    def __init__(self):  # default values hard coded for XeCTC acquisition at CCHMC
        ...
        self.trajorder = 2  # trajectory ordering for radial acqusitions,3 is Haltoned Spiral with multiple interleaves, 2 is halton randomized spiral 1 interleaves, 1 is 2D golden means, 0 is stock Philips
    ...
    def update(self, dl, rls, mrdHeader):
        ...
        if int(rls.header['sin']['nr_mixes'][0][0]) > 1:
            ...
            if self.exclude_bonus_spec:
                ...
                self.gas_contam_removal = False #True  # default to true if bonus spec present
        '''
