...
from Xe_CTC_GasEx_Config import Config, DataType
...
if data_set_config.data_type != DataType.UTE: # xenon has two trs for two frequencies
    pars.TR.insert(0,float(rls.header['sin']['repetition_times'][0][0]) * data_set_config.tr_factor) # tr_factor accounts for different frequencies
    pars.TR.insert(1,float(rls.header['sin']['repetition_times'][0][0]) * data_set_config.tr_factor)
...
if data_set_config.data_type != DataType.UTE: # xenon needs center freq and offset
    ...
    centFreq.value = float(rls.header['sin']['acq_gamma'][0][0]) / H1_GAMMA * float(exp.H1resonanceFrequency_Hz)
    userParams.userParameterLong.insert(0, centFreq)
    offFreq = mrd.xsd.userParameterLongType('xe_dissolved_offset_frequency')
    offFreq.value = float(rls.header['sin']['acq_gamma'][0][0]) / H1_GAMMA * float(exp.H1resonanceFrequency_Hz) * data_set_config.xe_dissolved_offset_ppm / 1000000
...
if data_set_config.data_type == DataType.DIXON or DataType.UTE:
    ...
    enc.encodingLimits.kspace_encoding_step_1.maximum = ((enc.encodingLimits.kspace_encoding_step_2.maximum+1) * temp_max_spokes_per_intlv) - 1
...
if data_set_config.data_type != DataType.UTE: # repetitions are instead contrast with proton/gas/dissolved as 0/1/2
    header.encoding[0].encodingLimits.contrast.minimum = min(data_set_config.contrast_order)
    header.encoding[0].encodingLimits.contrast.maximum = max(data_set_config.contrast_order)
...
header.encoding[0].encodingLimits.set.maximum = int(rls.header['sin']['nr_echoes'][0][0])
...
if data_set_config.multi_echo:
    for echo_idx in range(1, int(rls.header['sin']['nr_echoes'][0][0])-1): # all echoes after first will be full projections
        header.encoding.append(copy.deepcopy(header.encoding[0]))
        header.encoding[echo_idx].encodingLimits.kspace_encoding_step_0.minimum = -(header.encoding[echo_idx].encodingLimits.kspace_encoding_step_0.maximum+1)    
        header.encoding[echo_idx].encodedSpace.matrixSize.x = int(header.encoding[0].encodedSpace.matrixSize.x * 2)
...
for acqnum in range(dset.number_of_acquisitions()):
    ...
    if data_set_config.bonus_spec == True: # set flag for bonus spectra (prior to reusing set for echoes)
        acq_temp.measurement_uid = acq_temp.idx.set

    if data_set_config.ext_traj == True:
        try:
            if acq_temp.idx.contrast == 0:
                traj = crds[acq_temp.idx.kspace_encode_step_2, acq_temp.idx.kspace_encode_step_1, :, :]
            else:
    ...
    # remove interleaving
    if data_set_config.data_type == DataType.DIXON:
        acq_temp.idx.kspace_encode_step_1 = acq_temp.idx.kspace_encode_step_2 * temp_max_spokes_per_intlv + acq_temp.idx.kspace_encode_step_1
...
if data_set_config.data_type == DataType.CALIBRATION:
    try:
        os.remove(os.path.join(mrdName.parent, patientID+'_calibration.h5'))
    except:
        pass
    os.rename(mrdName, os.path.join(mrdName.parent, patientID+'_calibration.h5'))
if data_set_config.data_type == DataType.DIXON:
    try:
...
if data_set_config.data_type == DataType.UTE:
    try:
        ...
        pass
    os.rename(mrdName, os.path.join(mrdName.parent, patientID+'_proton.h5'))