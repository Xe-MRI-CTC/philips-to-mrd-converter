...
def decodeMira(bytebuff, temp_data, total_data_size):
    temp_data_size = 0
    index = 0  # track position in bytebuff

    while temp_data_size < total_data_size:
        data_size = getShort(bytebuff[index:index+2])
        index += 2
        tmp_coded_size = getShort(bytebuff[index:index+2])
        index += 2
        offset1 = getInt(bytebuff[index:index+4])
        index += 4

        temp_data_size += data_size

        read_cnt, assn_cnt = decode(
            bytebuff[index:index+tmp_coded_size],
            temp_data,
            offset1 // 4,
            data_size // 4
        )
