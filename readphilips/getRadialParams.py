...
def processRadialParams(header, trajtype, delay):
    ...
    if header['headerType'] == 'lab-sin':
        ...
        if 'k_space_traj_type' in hdr:
            # k_space_traj_type = 1 for radial, 2 for spiral.
            radial_traj = (int(float(hdr['k_space_traj_type'][0][0])) == 1)
        else:
...
# =============================================================================
    if radial_traj is True:
        ...
        try:
            fid_samp = int(float(hdr['non_cart_fid_sampling'][0][0]))
        ...
        if fid_samp:  # UTE ramp sampling
            ...
            try:
                ntrail = np.maximum(0, int(np.ceil(-float(hdr['non_cart_fid_trailing_sample'][0][0]))))
        ...
        if fid_samp:  # UTE ramp sampling
            ...
            int_ratio_1 = int(np.ceil(FILTER_TAPS * smoother - delay))
            ...
            # Apply the filter
            if smoother > 0:
                filt = np.exp(-np.arange(int(np.ceil(FILTER_TAPS * smoother))
                              + 1) / smoother)
            ...
            # Matching matlab cumtrapz function
            kr = np.asarray(np.concatenate(([0.0], np.cumsum(0.5 * (
                            grad_filt[int_ratio_1-1:int_ratio_1+nsamp+ntrail][1:] +
                            grad_filt[int_ratio_1-1:int_ratio_1+nsamp+ntrail][:-1])))))

            norm_fact = 0.5 * ((nsamp + ntrail - 1) / ((nsamp + ntrail) * kr[nsamp + ntrail - 1]))
        ...
        if mode == 0:  # 2D
            ...
            if fid_samp:
                if trajtype == 0:  # Standard Philips
                    print("Inside getRadialParams: trajtype=Standard Philips")
                    ...
                    print("Inside getRadialParams: trajtype=2D Golden Means")
                    phi1 = 0.46557123
                    phi2 = 0.6823278
