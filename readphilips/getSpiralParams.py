...
def processSpiralParams(header, filename, scanner):
    ...
    if header['headerType'] == 'lab-sin' and spiral_traj:
        ...
        if os.path.isfile(defpath) and ('ad_hoc_float_array' in sin or 'ad_hoc_int_array' in sin):
            ...
            print("WARNING: No ad_hoc values were found in sin file. The necessary information"
        ...
        try:
            spparams['COORDS_EXPANDED'] = rotatespirals(readSpiralCoords(filename, header, scanner), int(spparams['NR_SPIRAL_ARMS']), int(spparams['SPIRAL_SAMP']))
        except:
            print("Warning: An exception occurred in generating the expanded spiral coordinates.")
    
        print('''
              NOTE: The following components have not been ported from ExpandSpiralCoords:
              - x, y, and z delay times.
              - fractional signal averages (FNSA).
              - FLORET spiral coordinate generation. 
              ''')
    return(spparams)
...
def rotatespirals(coord_array, n_spirals, n_points):
    ...
    for jj in range(n_spirals): # spiral arms
        ...
        for ii in range(n_points): # points along spiral arm
            coord_array_expanded[0,ii,jj] = cb*coord_array[0,ii] - sb*coord_array[1,ii]
            coord_array_expanded[1,ii,jj] = cb*coord_array[1,ii] + sb*coord_array[0,ii]
    return(coord_array_expanded)

def fillspirals():
    print('WARNING: The current Python script does not have functionality for FLORET')
